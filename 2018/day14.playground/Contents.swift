...
var recipelists = [[5,1,5,8,9], [0,1,2,4,5], [9,2,5,1,0], [5,9,4,1,4], [3,0,6,2,8,1]]

func checkTargetRecipes() {
    for (idx, recipe) in recipelists.enumerated() {
        if Array(recipes.suffix(recipe.count)) == recipe {
            print("Recipe list \(recipe) found at \(recipes.count - recipe.count)")
            recipelists.remove(at: idx)
        }
    }
}
