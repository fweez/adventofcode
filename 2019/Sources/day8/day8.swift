...
public func part1() { print("Part 1: \(getMagicNumber(ingestFile("day8.txt").first!))") }
public func part2() { print("Part 2: \n\(printableSIF(ingestFile("day8.txt").first!))") }

func getMagicNumber(_ s: String.SubSequence) -> Int {
    ///To make sure the image wasn't corrupted during transmission, the Elves would like you to find the layer that contains the fewest 0 digits. On that layer, what is the number of 1 digits multiplied by the number of 2 digits?
    let layer = layers(s)
        .map { (a: [Int]) -> [Int] in a.filter { $0 != 0 } }
        .map { (a: [Int]) -> (Int, Int, Int) in (a.count, a.filter({ $0 == 1 }).count, a.filter({ $0 == 2 }).count) }
...
func printableSIF(_ s: String.SubSequence) -> String {
    layers(s)
